...
exports.getRecentFiles = async (req, res, next) => {
    try {
        ...
        const userId = req.user?.id || '507f1f77bcf86cd799439011'; // Default user ID for testing when not authenticated

        let query = { userId }; // Filter by user
...
exports.getJobDetails = async (req, res, next) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(jobId)) return res.status(400).json({ success: false, message: 'Invalid Job ID format.' });
...
exports.deleteJob = async (req, res, next) => {
    try {
        ...
        if (!jobToDelete) return res.status(404).json({ success: false, message: "Job not found." });

        console.log(`Deleted job record: ${jobId}`);
        res.status(200).json({ success: true, message: "Job deleted successfully." });
    } catch (error) { console.error("❌ Error in deleteJob:", error); next(error); }
};
...
exports.renameJob = async (req, res, next) => {
    try {
        ...
        const updatedJob = await Job.findByIdAndUpdate(jobId, { originalName: newName.trim() }, { new: true }).lean();
    ...
    } catch (error) { console.error("❌ Error in renameJob:", error); next(error); }
};
...
exports.retryJob = async (req, res, next) => {
    try {
        const jobId = req.params.jobId;
        if (!mongoose.Types.ObjectId.isValid(jobId)) return res.status(400).json({ success: false, message: 'Invalid Job ID.' });
        const jobToRetry = await Job.findOneAndUpdate({ _id: jobId, status: 'failed' }, { status: 'waiting', errorMessage: null }, { new: true });
        if (!jobToRetry) return res.status(404).json({ success: false, message: "Job not found or not in 'failed' state." });
    ...
    } catch (error) { console.error("❌ Error in retryJob:", error); next(error); }
};
